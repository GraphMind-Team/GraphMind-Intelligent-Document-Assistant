...
- **Trigger:** A second human-requested review pass ("search for any weaknesses and optimizations") after the first round of fixes, all tests green.
- **User-visible bug fixed (the significant one):** `DocumentsPage`'s `POLLABLE_STATUSES` was `['Uploaded']` — correct for Story 2.3, where a parsed document parked at `Extracting` forever, and explicitly reasoned as such in that file's comment. Story 2.4 is the story that invalidates it: the grid stopped polling at the *first* transition, so a document silently finished `Graphing → Ready` in the background while the library showed "Extracting" until a manual reload. This made 2.4's entire user-facing payoff — including the `chapter_breakdown` panel added by the first amendment — unreachable without a page refresh. Now polls every non-terminal status (`Uploaded`/`Extracting`/`Graphing`), with `MAX_POLL_ATTEMPTS` raised 15 → 45 (60s → 3min), since the attempt budget spans the whole pipeline (the pollable-id key doesn't change across transitions, so the effect never restarts mid-ingestion) and a cold fastembed download plus the LLM's 2×30s retry budget can legitimately exceed 60s. The existing test that pinned the old behavior ("stops once it leaves Uploaded") was rewritten; both new tests were verified to fail against the pre-fix code before being kept.
- **Optimization — Neo4j round-trips, ~90 → at most 6:** `_write_entities_and_relationships_tx` issued one `tx.run` per entity and per relationship, i.e. one network round-trip each to Aura. Now batched via `UNWIND`: a single statement for all entities, plus one per *distinct relationship type* (the relationship type is Cypher syntax and can't be parameterized, so it's the only thing forcing more than one statement — and OD-1 closes that vocabulary at five). Bounded at six queries regardless of document size. New test pins the grouping.
- **Optimization — extraction budget no longer wastes ~16% on duplicated text:** chunks overlap by `CHUNK_OVERLAP_WORDS` by design (Story 2.3, so a passage straddling a boundary stays retrievable whole), but `_build_extraction_text` concatenated them naively, re-sending those words to the LLM. The overlap is now stripped back off at same-chapter boundaries only (a new chapter's first chunk carries none), buying real document coverage from the same 12,000-char budget. `parsing._CHUNK_OVERLAP_WORDS` promoted to public `CHUNK_OVERLAP_WORDS` so both sides read one source of truth, mirroring `weaviate_client.PASSAGE_BATCH_SIZE`'s precedent. Three new tests.
- **Optimization — up to 20MB released before the slow path:** `db.expire(document, ["content"])` right after parsing. The session's identity map held the raw upload until `db.close()`, which since this story spans the embedding loop *and* the LLM call (up to 60s on retries) — the least memory-headroom stretch of the pipeline, on a 512MB instance that permits concurrent uploads.
- **Minor:** a 2s pause between LLM retry attempts (retrying a transient failure in the same microsecond mostly re-hits it, spending the second attempt for nothing; safe to sleep in the background threadpool), and `.consume()` on Neo4j results so a per-statement failure surfaces at its own statement rather than at commit/session-close.
- **KEEP:** every boundary and both prior Change Log entries above; no Intent/Boundaries/I-O-Matrix commitment is altered by any of this.

- **Trigger:** First real upload against live services failed at the Graphing step. Diagnosed by calling `extract_entities_and_relationships` directly with the real API key rather than reading logs.
- **Root cause (deploy-blocking):** the default model `meta-llama/llama-3.3-70b-instruct:free` now returns `404 "This model is unavailable for free. The paid version is available now"`. It was never exercised against the live provider before this — every test mocks `httpx.post` — so the story shipped with a default that could not work. Replaced with `openai/gpt-oss-20b:free`, chosen by measuring candidates from OpenRouter's live free-tier list against this module's actual prompt (correct object-shaped entities *and* relationships, English and Bulgarian, fastest of the correct ones). `openrouter/free` was rejected on measurement: it returns `entities` as bare strings, which `_parse_and_validate` correctly drops, leaving relationships with nothing to resolve against and an empty graph.
- **Second bug, found by the same run and more important than the model swap:** 429 was classified non-retryable, because the code treated *all* 4xx that way. On the free tier — which this project is constrained to by AD-8 — "temporarily rate-limited upstream, please retry shortly" is a routine response, so ingestion would have failed documents for a condition that clears by itself seconds later. 429 is now retryable (every other 4xx still isn't), honouring the server's `Retry-After` when present.
- **Amended boundary (was "2 attempts total"):** now 3 attempts with a doubling backoff (3s, 6s, capped at 30s), because a single retry against a provider that rate-limits in bursts still fails documents that would have succeeded. This narrows a frozen Boundaries line, so it is recorded here rather than edited silently above.
- **Timeout raised 30s → 120s:** measured, not guessed — a *one-sentence* prompt took ~32s end to end against the real provider, and real ingestion sends up to 12,000 characters (a 5.5k-char payload took 44s). The old value sat below observed latency for healthy requests.
- **Also required, and not a code issue:** the Alembic migration had never been run against the real Neon database, so `GET /documents` 500'd with `column documents.chapter_breakdown does not exist` while the whole test suite passed (tests build SQLite schemas from the models directly, bypassing migrations entirely). Applied with `alembic upgrade head`. This is the exact drift the "migration-vs-model" entry in `deferred-work.md` predicts, now demonstrated rather than theorised.
- **Verified live, end to end:** a real document reached `Ready` with `chapter_breakdown` populated; Bulgarian text extracted correct Cyrillic entities with correct types and a `RELATED_TO` fallback; `ensure_ready()` created the uniqueness constraint against real Aura; and AD-4's merge semantics were confirmed against a live database (an entity repeated across two documents produced one node, "TechCorp" vs "TechCorp Supplies" stayed distinct, a repeated relationship produced one edge) — the guarantee the unit tests can only simulate with a fake transaction recorder.
