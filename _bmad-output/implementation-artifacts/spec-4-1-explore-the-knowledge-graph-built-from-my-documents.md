...
- `frontend/package.json` -- edit: `force-graph`, `react-kapsule` added as explicit direct dependencies (both already present transitively via `react-force-graph`, which stays listed too -- see Design Notes)
