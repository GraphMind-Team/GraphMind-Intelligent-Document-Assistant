...
- Given a `Failed` document, when Document Detail loads, then no field promises the document is still being parsed/indexed — Chapters, Passages indexed, and the chapter breakdown all read as terminal, not pending.

## Spec Change Log

- **Trigger:** A second, independent human-requested review after the workflow's own three review layers had already passed and the story was committed. Both findings below were missed by all three of those layers.
- **Deploy-blocking, found and fixed:** the Alembic migration adding `documents.failed_reason` had never been applied to the real Neon database (`alembic current` = `c7d2a4f8e6b1`, `alembic heads` = `d4e9b1f3a7c2`; an `information_schema` query confirmed the column was absent). Because `failed_reason` is on `DocumentResponse`, which *all three* document routes use, SQLAlchemy would have selected the missing column on every request — upload, list, and detail would all have 500'd on deploy. The full suite stayed green throughout because `conftest.py` builds the SQLite schema from the ORM models directly and never runs a migration. This is the **second** occurrence of this exact failure mode (Story 2.4's change log records the first, with `chapter_breakdown`), and `deferred-work.md` already predicts it — the open CI item to run `alembic upgrade head` against a real Postgres is the durable fix. Resolved here by applying the migration and verifying the column now exists and `alembic current` == head.
- **User-visible contradiction, found and fixed:** a `Failed` document's Detail page rendered "Chapters: Pending", "Passages indexed: Pending", and "Pending — the chapter breakdown appears once this document has been parsed and indexed." *directly above* the failure reason. Ingestion is terminal (no retry endpoint exists), so that copy is a promise the app cannot keep, and it contradicts the very reason rendered beneath it — the opposite of this story's purpose, and a breach of the epic's stated voice rule ("plain, declarative, specific about why; no hedging"). UX-DR8's "Pending, never a fabricated 0" rule was written for in-flight documents and was being over-applied to a terminal one. Fixed by splitting the non-Ready fallback: `Failed` renders "Unavailable" / "Unavailable — this document failed to process."; every other non-Ready status keeps "Pending" verbatim. Verified by dumping the rendered `textContent` before and after, and pinned by two new tests (one asserting the Failed page never shows "Pending" or the "appears once…" copy, one asserting an in-flight `Graphing` document still does).
- **KEEP:** every frozen boundary above, the single-commit `status`/`failed_reason` invariant, the four stage labels and their positions (including `stage` flipping to extraction *before* the Graphing-status commit), and the Detail-only placement decision — none are altered by this entry.
