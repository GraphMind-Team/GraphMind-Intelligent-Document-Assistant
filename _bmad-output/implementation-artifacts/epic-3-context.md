...
- The window's size is resolved (OD-8): `HISTORY_MAX_TURNS = 3` / `HISTORY_MAX_CHARS = 2000` in `shared/llm_client/__init__.py`, recorded as configuration beside `RELEVANCE_THRESHOLD` rather than hardcoded, consistent with how OD-2 was resolved. The basis is Story 3.4's live manual verification, not an instrumented sweep — see `epics.md`'s OD-8 row and `deferred-work.md` for what that evidence does and doesn't cover.
...
- Epic 6's evaluation harness (FR-13) invokes this epic's service layer directly and measures SM-1/SM-2/SM-C1 against it; OD-2's threshold value came from that harness's baseline run and is flagged for re-tuning as the evaluation set grows. The harness calls `ask_question(..., use_history=False)`: it runs its whole question set sequentially through one QA account, so leaving Story 3.4's conversational memory on would fold the previous three unrelated questions into every question's retrieval embedding and generation prompt, and persisted rows would leak between runs — either alone would mean the harness no longer measures what OD-3's baseline measured. Any future caller that runs questions in a batch against one account has the same obligation.
