...
- Design tokens (light / dark) are the single source of truth for color — no component hardcodes a raw hex value. Three token gaps are explicitly assigned to this epic to close before component work begins elsewhere:
  - Citation chip pair (`#4A7FE0` on `#D1EEFE`) ships as a knowingly accepted AA contrast deviation in light mode — use as specified, do not re-tune.
