...
- Supported formats are PDF, Markdown, HTML, DOCX and PPTX; unsupported formats are rejected with a clear reason before any processing starts, and files over 20MB are rejected with a reason naming the limit. There is no cap on document count per user. **This line intentionally diverges from the planning artifacts it was compiled from** — `epics.md` and `prd.md` still say "PDF, Markdown, HTML only", which was true when they were written and is deliberately left as their own frozen pre-implementation intent. DOCX/PPTX were added later by direct request (`1b3354f`); see `spec-2-1`'s Spec Change Log for the amended allowlist and `backend/app/documents/service.py`'s `_EXTENSION_TO_FILE_TYPE` for the authoritative list. A `compile-epic-context` regeneration will silently revert this sentence to the stale planning wording — re-apply it rather than trusting the regenerated text.
