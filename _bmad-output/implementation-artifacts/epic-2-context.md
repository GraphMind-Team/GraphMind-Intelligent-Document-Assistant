...
- Status pills use one shared token pair (tint + text label) per state, color never standing alone, label as real selectable DOM text (not an icon-font glyph or pseudo-element). Note: as of the last accessibility review, tint tokens were fully specified for only 2 of the 5 states and this gap is still blocking — confirm all five have a token pair clearing 4.5:1 contrast before shipping pill-rendering surfaces in this epic.
...
- OD-1 (the entity/relationship type list) is now resolved, so Story 2.4's extraction-prompt work is unblocked.
