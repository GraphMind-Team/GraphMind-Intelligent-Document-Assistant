...
- Entity/relationship extraction merges into one unified graph per user (not a graph per document); matching entities merge, non-matches stay distinct. Extraction is constrained to a fixed, closed type set — the exact list is an open decision (see below) and blocks the extraction story.
...
- **Open blocker (OD-1):** the fixed entity/relationship type list for extraction is not yet decided. Story 2.4 (extraction) cannot have its prompts written until this is resolved — treat it as a hard prerequisite, not a nice-to-have.
...
- Status pills use one shared token pair (tint + text label) per state, color never standing alone, label as real selectable DOM text (not an icon-font glyph or pseudo-element). Note: as of the last accessibility review, tint tokens were fully specified for only 2 of the 5 states — confirm all five have a token pair clearing 4.5:1 contrast before shipping pill-rendering surfaces in this epic.
...
- Story 2.4 is explicitly blocked on OD-1 (the entity/relationship type list) — do not start extraction-prompt work until that decision lands.
