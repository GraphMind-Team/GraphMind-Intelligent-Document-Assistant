...
| OD-8 | ✅ RESOLVED | **Conversational-memory window size (FR-17).** `HISTORY_MAX_TURNS = 3` / `HISTORY_MAX_CHARS = 2000` (`shared/llm_client/__init__.py`), budgeted entirely separately from `_MAX_PROMPT_CHARS`'s 12,000-character passage block so neither silently eats the other's allowance. Basis is Story 3.4's manual verification against real OpenRouter/Weaviate/Postgres — history-augmented follow-ups resolved references correctly with no observable answer-quality or latency regression — **not** an instrumented sweep across window sizes, and deliberately not measurable by Epic 6's harness, which runs stateless (`use_history=False`) so its SM-1/SM-2/SM-C1 numbers stay comparable to OD-3's baseline. Weaker evidence than OD-2's retrieval-distance measurements; the instrumented measurement is carried in `deferred-work.md` for whenever free-tier context limits tighten or NFR-1 latency work resumes |
