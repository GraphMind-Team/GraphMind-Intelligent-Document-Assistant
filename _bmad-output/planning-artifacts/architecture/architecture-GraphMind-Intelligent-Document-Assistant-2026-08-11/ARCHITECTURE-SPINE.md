...
- **Exact entity/relationship type list for extraction (FR-5).** Genuinely still undecided. PRD's Open Question #1 (§8): flagged as architecture's decision but not resolved in this coaching session. Needed before extraction prompts can be written — revisit before that work starts.
